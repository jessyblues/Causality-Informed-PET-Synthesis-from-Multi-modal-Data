...
class CrossAttention(nn.Module):
    ...
    Args:
        ...
        ## notice!! cross_attention_dim: number of channels in the context.
...
class AttenUNet(nn.Module):
    ...
    def forward(
        ...
        for downsample_block in self.down_blocks:
            #pdb.set_trace()
            h, res_samples = downsample_block(hidden_states=h, context=context)
        ...
        # 5. mid
        h = self.middle_block(hidden_states=h, context=context)
        #pdb.set_trace()
        # Additional residual conections for Controlnets
        ...
        for upsample_block in self.up_blocks:
            ...
            h = upsample_block(hidden_states=h, res_hidden_states_list=res_samples, context=context)
...
if __name__ == '__main__':
    ...
    atten_unet = AttenUNet(spatial_dims=3, in_channels=1, out_channels=1, cross_attention_dim=3, with_conditioning=True,
                           num_res_blocks=(1, 1, 1), num_channels=(8, 16, 16), norm_num_groups=8, 
                           attention_levels=[False, False, True]).to(device)
    optimizer = torch.optim.Adam(params=atten_unet.parameters())
    loss = nn.L1Loss()
    img = torch.ones((1, 1, 92, 128, 92), device=device)
    context = torch.ones((1, 3), device=device)
    img2 = atten_unet(img, context)
    #pdb.set_trace()
    loss_ = loss(img2, torch.ones((1, 1, 92, 128, 92), device=device))
    optimizer.zero_grad()
    loss_.backward()
    optimizer.step()
    print(img.shape)
